// TRV™ vs SHA3-256 Pure Software Rigorous Benchmark
// Compiled under native release optimizations (-O3) to analyze peak software throughput.
...
fn keccak_f1600(state: &mut [u64; 25]) {
    ...
    for round in 0..24 {
        ...
        for _ in 0..24 {
            ...
            let rot_offset = R[x + 5 * y]; // Use original cell's rotation offset!
...
fn main() {
    println!("==================================================");
    println!("   🛡️ SHA3-256 vs. TRV™ MONOLITH PURE BENCHMARK  ");
    ...
    // SHA3-256 Verification against official test vectors (empty string)
    let sha3_empty = sha3_256(b"");
    let sha3_expected = "a7ffc6f8bf1ed76651c14756a061d662f580ff4de43b49fa82d80a4b80f8434a";
    let sha3_hex = to_hex(&sha3_empty);
    assert_eq!(sha3_hex, sha3_expected, "SHA3-256 Empty string hash mismatch!");
    println!("  ✅ SHA3-256 Self-Test: Verification SUCCESS!");
    
    // TRV™ Hash Verification
    let trv_empty = trv_hash(b"");
    let trv_expected = "00ce8464f821b55c31db486ec711c316f61c73712682c3504712ba7cc3504bc1";
    let trv_hex = to_hex(&trv_empty);
    assert_eq!(trv_hex, trv_expected, "TRV-Hash Empty string hash mismatch!");
    println!("  ✅ TRV-Hash Self-Test: Verification SUCCESS!");
    println!();

    // Step 2: Set payload size (1024MB for 1GB benchmark)
    ...
    // --- SHA3-256 BENCHMARK ---
    println!("[ BENCHMARK ] Running pure SHA3-256 sponge...");
    let start = Instant::now();
    let sha3_res = sha3_256(&payload);
    let sha3_duration = start.elapsed();
    let sha3_mbps = (size as f64 / 1024.0 / 1024.0) / sha3_duration.as_secs_f64();
    
    // --- TRV™ MONOLITH BENCHMARK ---
    println!("[ BENCHMARK ] Running pure TRV™ Monolith hash...");
    let start = Instant::now();
    let trv_res = trv_hash(&payload);
    let trv_duration = start.elapsed();
    let trv_mbps = (size as f64 / 1024.0 / 1024.0) / trv_duration.as_secs_f64();
    
    // Step 3: Print structured comparative metrics
    println!("\n==================================================");
    println!("  📊 DETAILED PURE SOFTWARE COMPARISON (-O3)      ");
    println!("==================================================");
    println!("  SHA3-256:");
    println!("    Time taken:  {:?}", sha3_duration);
    println!("    Throughput:  {:.2} MB/s", sha3_mbps);
    println!("    Check:       {:02x?}", &sha3_res[..4]);
    println!();
    ...
    println!("  🚀 Result: TRV™ is {:.2}x FASTER than SHA3-256", speedup);
    println!("==================================================");
}
